 ...
 public void run (ImageProcessor ip){
	...
	for(int y=1; y<hA-3; y++){
		for(int x=1; x<wA-2; x++){
			int min = 2000;
			int val = 0;
			val =  DM[x-1][y-1] + 1; //les y sont inversés par rapport à la formule du cours car l'axe y est vers le bas
			if(val<min)min = val;
			val = DM[x][y-1] + 1;
			if(val<min)min = val;
			val = DM[x+1][y-1] + 1;	
	...
	for (int i = 1; i<wA-1; i++) {
		for (int j = 1; j<hA-1; j++) {
			...
			if(value>0){
				boolean isMax = true;
				//si il a une distance > à celle de ses voisins de la région en d8, alors c'est un point de la médiane
				if(DM[i-1][j-1]>0 && value<DM[i-1][j-1]) isMax = false;
				...
				/*if(value>=DM[i-1][j-1] && value>=DM[i-1][j] && value>=DM[i-1][j+1] && value>=DM[i][j-1] && value>=DM[i][j+1] && value>=DM[i+1][j-1] && value>=DM[i+1][j] && value>=DM[i+1][j+1]){
